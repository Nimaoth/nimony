import std/[macrocache, json, options, tables, strutils, strformat, sequtils, assertions]
...
type
  ...
  WasmImportDesc* = object
    ...
    of WasmImportDescKind.Func:
      funcTypeIdx*: WasmTypeIdx
    of WasmImportDescKind.Table:
      table*: WasmTableType
    of WasmImportDescKind.Mem:
      mem*: WasmMemoryType
    of WasmImportDescKind.Global:
  ...
  WasmExportDesc* = object
    ...
    of WasmExportDescKind.Func:
      funcIdx*: WasmFuncIdx
    of WasmExportDescKind.Table:
      tableIdx*: WasmTableIdx
    of WasmExportDescKind.Mem:
      memIdx*: WasmMemIdx
    of WasmExportDescKind.Global:
...
proc addImport*(self: WasmBuilder, module: string, name: string, inputs: openArray[WasmValueType], outputs: openArray[WasmValueType]): WasmFuncIdx =
  let typeIdx = self.types.len.WasmTypeIdx
  self.types.add WasmFunctionType(input: WasmResultType(types: @inputs), output: WasmResultType(types: @outputs))
  self.addImport(module, name, typeIdx)
...
proc addExport*(self: WasmBuilder, name: string, tableIdx: WasmTableIdx) =
  self.exports.add WasmExport(
    name: name,
    desc: WasmExportDesc(kind: WasmExportDescKind.Table, tableIdx: tableIdx)
  )
...
proc addGlobal*[T](self: WasmBuilder, typ: WasmValueType, mut: bool, init: T, id: string = ""): WasmGlobalIdx =
  ...
  of V128: (assert(false, "V128 not supported"); WasmInstr(kind: Nop))
  of FuncRef: (assert(false, "FuncRef not supported"); WasmInstr(kind: Nop))
  of ExternRef: (assert(false, "ExternRef not supported"); WasmInstr(kind: Nop))
...
proc getFunctionType*(self: WasmBuilder, funcIdx: WasmFuncIdx): WasmFunctionType =
  return self.types[self.getFunctionTypeIdx(funcIdx).int]
...
proc localName*(self: WasmBuilder, funcIdx: WasmFuncIdx, localIdx: WasmLocalIdx): string =
  let typeIdx = self.funcs[funcIdx.int].typeIdx.int
  let typ = self.types[typeIdx]
  let numParams = typ.input.types.len
  if localIdx.int in 0..<numParams:
    return "p" & $localIdx
  elif localIdx.int - numParams in 0..self.funcs[funcIdx.int].locals.high:
    return self.funcs[funcIdx.int].locals[localIdx.int - numParams].id
  else:
    return "var" & $localIdx

proc getInstrWat(self: WasmBuilder, instr: WasmInstr, funcIdx: WasmFuncIdx, blockLevel: int = 0): string =
  result = ""
  case instr.kind
  of Block:
    ...
    for subInstr in instr.blockInstr:
      result.add "\n"
      result.add getInstrWat(self, subInstr, funcIdx, blockLevel + 1).indent(1, "  ")
  ...
  of Select:
    ...
    if instr.selectValType.isSome:
      result.add " (result "
      result.add $instr.selectValType.get
      result.add ")"

  of LocalGet:
    let id = self.localName(funcIdx, instr.localIdx)
    result.add &"local.get ${id} (;{instr.localIdx};)"
  of LocalSet:
    let id = self.localName(funcIdx, instr.localIdx)
    result.add &"local.set ${id} (;{instr.localIdx};)"
  of LocalTee:
    let id = self.localName(funcIdx, instr.localIdx)
    result.add &"local.tee ${id} (;{instr.localIdx};)"
  of GlobalGet:
    let effectiveIdx = getEffectiveGlobalIdx(self, instr.globalIdx)
    let id = if effectiveIdx in 0..self.globals.high:
      self.globals[effectiveIdx].id
    else:
      "var" & $instr.globalIdx
    result.add &"global.get ${id}"
  of GlobalSet:
    let effectiveIdx = getEffectiveGlobalIdx(self, instr.globalIdx)
    let id = if effectiveIdx in 0..self.globals.high:
      self.globals[effectiveIdx].id
  ...
  of Comment:
    result.add "(; "
    result.add instr.comment
    result.add " ;)"

proc getExprWat(self: WasmBuilder, exp: WasmExpr, funcIdx: WasmFuncIdx): string =
  result = ""
  for i, instr in exp.instr:
    if i > 0: result.add "\n"
    result.add self.getInstrWat(instr, funcIdx, 0)
...
proc getFunctionSectionWat(self: WasmBuilder): string =
  ...
  for i, v in self.funcs:
    ...
    result.add "\n"
    result.add getExprWat(self, v.body, i.WasmFuncIdx).indent(1, "  ")
...
proc getGlobalSectionWat(self: WasmBuilder): string =
  ...
  for i, v in self.globals:
    result.add "\n"
    let init = self.getExprWat(v.init, 0.WasmFuncIdx)
...
proc getStartSectionWat(self: WasmBuilder): string =
  result = ""
  if self.start.isSome:
    result.add &"\n(start {getEffectiveFunctionIdx(self, self.start.get.start)})"
...
proc getElementListWat(self: WasmBuilder, typ: WasmValueType, exprs: openArray[WasmExpr]): string =
  ...
  for e in exprs:
    result.add " (item "
    result.add self.getExprWat(e, 0.WasmFuncIdx)
    result.add ")"
...
proc getElementSectionWat(self: WasmBuilder): string =
  ...
  for i, v in self.elems:
    ...
    of Active:
      result.add fmt"(elem (table {getEffectiveTableIdx(self, v.mode.tableIdx)}) (offset {getExprWat(self, v.mode.offset, 0.WasmFuncIdx)}) {getElementListWat(self, v.typ, v.init)})"
    of Declarative:
...
proc getDataSectionWat(self: WasmBuilder): string =
  ...
  for i, v in self.datas:
    ...
    of Passive:
      discard
    of Active:
      result.add fmt"(memory {v.mode.memIdx}) (offset {getExprWat(self, v.mode.offset, 0.WasmFuncIdx)}) "
...
proc getStackChange*(self: WasmBuilder, instr: WasmInstr): int =
  ...
  of I32TruncSatF32S, I32TruncSatF32U, I32TruncSatF64S, I32TruncSatF64U, I64TruncSatF32S, I64TruncSatF32U, I64TruncSatF64S, I64TruncSatF64U: return 0
    ...
    # todo
    echo fmt"getStackChange: unhandled instr {instr.kind}"
    return 0
...
proc getInstrType*(self: WasmBuilder, instr: WasmInstr): WasmFunctionType =
  ...
  of Block:
    ...
    of ValType:
      if instr.blockType.typ.isSome:
        WasmFunctionType(
          input: WasmResultType(types: @[]),
          output: WasmResultType(types: @[instr.blockType.typ.get]),
        )
      else:
        WasmFunctionType()
  ...
  of Loop:
    ...
    of ValType:
      if instr.loopType.typ.isSome:
        WasmFunctionType(
          input: WasmResultType(types: @[]),
          output: WasmResultType(types: @[instr.loopType.typ.get]),
        )
      else:
        WasmFunctionType()
  ...
  of If:
    ...
    of ValType:
      if instr.ifType.typ.isSome:
        WasmFunctionType(
          input: WasmResultType(types: @[]),
          output: WasmResultType(types: @[instr.ifType.typ.get]),
        )
      else:
        WasmFunctionType()
...
proc validate*(self: WasmBuilder, instr: WasmInstr, expectedSize: Option[int], path: seq[int], doLog: bool): bool {.raises: [Exception].} =
  ...
  if expectedSize.isSome and self.getStackChange(instr) != expectedSize.get:
    result = false
...
when isMainModule:
  ...
  let f = module.get.findFunction("test export name", void, proc(a: int32, b: int64, c: float32, d: float64): void)
  if f.isSome:
